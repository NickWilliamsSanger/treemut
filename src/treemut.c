...
void likelihood_mtr(int *mtr,int *depth,int * geno, double * el, double * perr, int * nmuts, int * nsamp, int * nbranch,double * vaf,double * lik){
  int nm= *nmuts;
  int ns= *nsamp;
  int nb= *nbranch;
  double loglik=0.0;
  ...
  for(i=0;i<ns;i++){
    logV[i]=log(*vaf-perr[i]);
    logP[i]=log(perr[i]);
    log1minusV[i]=log(1-(*vaf)+perr[i]);
